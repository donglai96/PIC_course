...
def initialize_menus(pc):
    ...
    if in1.nts > 0:
        if (in1.nds==1) or (in1.nds==3):
           pc.addGraph("ELECTRON Vx vs X", "Electron/Electron Phase")  # Enable electron phase space
        if in1.movion:
            if (in1.nds==2) or (in1.nds==3):
                pc.addGraph("ION Vx vs X", "Ion/Ion Phase")  # Enable ion phase space
    if (in1.ntw > 0):
...
# display frequency spectrum
                  pc.showSimpleImage("ION DENSITY +OMEGA VS K",s1.pkwdi[1::,:,0],
                                     ...
                                     title='Ion Density +Omega vs K',early=in1.ntdi,
                  ...
                  pc.showSimpleImage("ION DENSITY -OMEGA VS K",s1.pkwdi[1::,:,1],
                                     ...
                                     title='Ion Density -Omega vs K',early=in1.ntdi,
                                     ticks_scale=spectrum_scale)
                  graf1.dmscaler1(s1.wkdi,'ION DENSITY OMEGA VS MODE',time,999,
...
# display frequency spectrum
            ...
            pc.showSimpleImage("POTENTIAL -OMEGA VS K",s1.pkw[::,:,1],
                               ...
                               title="Potential: -Omega vs K",early=in1.ntp,
                               ticks_scale=spectrum_scale)
            graf1.dmscaler1(s1.wk,'POTENTIAL OMEGA VS MODE',time,999,2,
...
# display longitudinal efield
            graf2.dscaler1(s1.sfield,'ELFIELD',time,999,0,nx,irc,
                           title='Longitudinal E-Field vs X',early=in1.ntel)
            if (irc[0]==1): break
            irc[0] = 0
...
# plot electrons vx versus x
            if ((in1.nds==1) or (in1.nds==3)):
               phaselabels = False
# Do we have labelled particles to draw?
               if in1.ntsc > 0:
                  phaselabels = True
               graf2.dpmgrasp1(s1.ppart,s1.kpic,'ELECTRON Vx vs X',ntime,999,nx,2,1,
                               in1.ntsc,irc,early=in1.nts,twophase=phaselabels )
               if (irc[0]==1): break
               irc[0] = 0
# ion phase space
            if (in1.movion==1):
...
# energy diagnostic: updates wt
      if (in1.ntw > 0):
         ...
         if (ntime==in1.ntw*it):
            ...
            pc.showEnergy(in1.ntw*numpy.array(range(it+1))*in1.dt,s1.wt,it,
                          ["Total Field","Kinetic","Kinetic Ions","Total Energy"],
                          title='Energy vs Time',early=in1.ntw)
